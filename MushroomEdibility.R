...
# we see high multicollinearity with stalk_color_below_ring and stalk_color_above_ring,
# as well as stalk_surface_below_ring and stalk_surface_above_ring, as expected
...
# eliminating multicollinear features: increases model accuracy (removes 3 features)
# from the correlation heatmap, the following variables are correlated with one another
# (denoted by unusually black and light spots in the heatmap)
# gill_color and gill_size
# spore_print_color and ring_type
# veil_color and gill_attachment
# we remove the ones with lower correlation with class label:
# gill_color, spore_print_color and gill_attachement
data = data[,!names(data) %in% c("gill_attachment", "ring_type")]

# selecting top 10 features for rpart (only uses 9 features)
print(ncol(data))
numerical_data = sapply(data, unclass)
correlation = cor(numerical_data)
...
# *Decision Tree
#par(mfrow=c(2,2)) # (for plots)

# penalty matrix: will set a penalty which is 10 times bigger for classifying a
...
# create confusion matrix (for TP, FP rate and accuracy etc)
...
X = as.matrix(train_numeric[,-1])
Y = as.matrix(train_numeric[,1])

# scaling training data (no need to standardize Y since it's 1 and 0)
#scaledTrain = as.data.frame(scale(train_numeric))
...
#scaledX <- scaledX %>% map_df(function(.x) as.factor(.x))
#Y <- Y %>% map_df(function(.x) as.factor(.x))
...
#plot(cap_surface ~ odor, data=scaledTrain)
#lines(scaledTrain$Age, glm.out$fitted, type="l", col="red")

# for one unit of cap_shape, we see log odds of edibility decrease by 70.848
# GRAPH OF |ESTIMATE| OF EACH (Intercept)
# Coefficient estimates are the theta
...
# *Visualizing predictors and class_label relationships 

# predicting popular poisonous mushrooms

# normalize numerical_data
#normalize = function(x){
#  return ((x - min(x))/(max(x) - min(x)))
#}
#numerical_data = apply(numerical_data, 2, normalize)
